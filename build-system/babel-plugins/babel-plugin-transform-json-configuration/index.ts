import type { PluginObj, Node, NodePath } from "@babel/core";
import type { CallExpression } from "@babel/types";

/**
 * @interface {babel.PluginPass}
 */
 export default function ({template, types: t}: any): PluginObj {
  /**
   * Produces a random number that is guaranteed not to be present in str.
   */
  function uniqInString(str: string): number {
    while (true) {
   ...
   *
   */
  function stringifyValue(path: NodePath): string {
    ...
    try {
      ...
      const ref = arg || path;
      throw (ref as NodePath<Node>).buildCodeFrameError(
  ...
  const handlers = Object.assign(Object.create(null), {
    jsonConfiguration(path: NodePath) {
      path.replaceWith(template.expression.ast`
        JSON.parse(${stringifyValue(path)})
      `);
    },

    includeJsonLiteral(path: any) {
      path.replaceWith(path.node.arguments[0]);
    },

    jsonLiteral(path: any) {
  ...
  return {
    ...
    visitor: {
      CallExpression(path: NodePath<CallExpression>) {
        const handler = handlers[(path.node.callee as any).name];
        if (handler) {
          handler(path);
        }
      },
